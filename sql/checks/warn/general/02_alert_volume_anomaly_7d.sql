WITH daily AS (
  SELECT DATE(alert_ts) AS dt, COUNT(1) AS cnt
  FROM `{{ params.project }}.{{ params.dataset }}.alerts`
  WHERE DATE(alert_ts) BETWEEN DATE_SUB(DATE('{{ params.run_date }}'), INTERVAL 7 DAY)
...
SELECT
  ...
  CONCAT('alert_deviation_ratio=', CAST(COALESCE(deviation_ratio, 0) AS STRING),
